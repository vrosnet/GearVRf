...
public:
    ...
    void setJavaOwner(JNIEnv& env, jobject javaObject) {
        ...
        javaMethodIsAvailable_ = GetMethodId(env, gvrTextureClass_, kIdAvailableMethodName,
    ...
    virtual void runPendingGL() {
        ...
        LOGI("mmarinov:runPendingGL: a");
        JNIEnv* env = getCurrentEnv(javaVm_);

        LOGI("mmarinov:runPendingGL: b");
        jobject textureObject = env->NewLocalRef(textureObjectWeak_);
        if (nullptr != textureObject) {
            LOGI("mmarinov:runPendingGL: c");
            env->CallVoidMethod(textureObject, javaMethodIsAvailable_, gl_texture_->id());
            LOGI("mmarinov:runPendingGL: d");
            env->DeleteLocalRef(textureObject);
            LOGI("mmarinov:runPendingGL: e");
        } else {
            LOGI("mmarinov:not available: ");
        }
    }
...
private:
    ...
    jmethodID javaMethodIsAvailable_;
};
